...
class ThunderDetector:
    ...
    def detect(self, chunk, threshold):
        ...
        if self.prev_avg==0:
            ...
            if mean(fcontent[self.minfbin:self.maxfbin])/self.prev_avg>threshold or (self.time_since_thunder==0 and mean(fcontent[self.minfbin:self.maxfbin])/self.prev_avg>threshold/10):
